...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/goose`).then(res=>{
...
const lab8 = () =>{
    axios.get(`http://localhost:3000/query-test/?asdf/? asdf`).then(res=> // Need ? before query or it breaks
        console.log(res.data))
    .catch(err=>console.log(err))
}

document.getElementById(`query-button`).addEventListener("click", lab8)
...
/*
    ...
    1: Send no queries on the URL -- what happened? Logged: "You sent an empty query!"

    2: Send more than 1 query on the URL -- what happened?  Logged: "You sent query: asdf/? asdf with value: !"
*/
...
/*
    ...
    Next, create an object called body inside the function. It should have one key-value pair. The key should be newFood (make sure to match the case and spelling exactly) and the value should 
